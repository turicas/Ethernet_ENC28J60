...
#define Sn_MR_TCP               0x01 //TODO: change to IP_PROTO_TCP

uint8_t socket(SOCKET s, uint8_t protocol, uint16_t port, uint8_t flag);

uint8_t listen(SOCKET s);

uint8_t connect(SOCKET s, uint8_t *address, uint16 port);

uint16_t send(SOCKET s, const uint8_t *buffer, uint16 length);

uint16_t recv(SOCKET s, const uint8_t *buffer, uint16 length);
